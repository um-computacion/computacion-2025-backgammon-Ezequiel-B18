...
class TestPlayer(unittest.TestCase):
    ...
    def test_use_move(self):
        ...
        self.assertEqual(self.white_player.remaining_moves, 0)
        self.assertTrue(self.white_player.is_turn)  # Still in turn until explicitly ended
    ...
    def test_get_checkers_by_state(self):
        ...
        self.assertEqual(len(self.white_player.get_checkers_by_state(CheckerState.ON_BOARD)), 15)
        self.assertEqual(len(self.white_player.get_checkers_by_state(CheckerState.ON_BAR)), 0)
        self.assertEqual(len(self.white_player.get_checkers_by_state(CheckerState.BORNE_OFF)), 0)
        ...
        # Check counts again
        self.assertEqual(len(self.white_player.get_checkers_by_state(CheckerState.ON_BOARD)), 12)
        self.assertEqual(len(self.white_player.get_checkers_by_state(CheckerState.ON_BAR)), 2)
        self.assertEqual(len(self.white_player.get_checkers_by_state(CheckerState.BORNE_OFF)), 1)

    def test_count_checkers_by_state(self):
        """Test counting checkers by their state"""
        # Initially all checkers are on board
        self.assertEqual(self.white_player.count_checkers_by_state(CheckerState.ON_BOARD), 15)
        self.assertEqual(self.white_player.count_checkers_by_state(CheckerState.ON_BAR), 0)
        self.assertEqual(self.white_player.count_checkers_by_state(CheckerState.BORNE_OFF), 0)
        ...
        # Check counts again
        self.assertEqual(self.white_player.count_checkers_by_state(CheckerState.ON_BOARD), 13)
        self.assertEqual(self.white_player.count_checkers_by_state(CheckerState.ON_BAR), 1)
        self.assertEqual(self.white_player.count_checkers_by_state(CheckerState.BORNE_OFF), 1)
    ...
    def test_player_str_representation(self):
        ...
        self.assertEqual(str(self.white_player), "Player 1 (WHITE): 15 on board, 0 on bar, 0 borne off, not in turn")
        ...
        self.assertEqual(str(self.white_player), "Player 1 (WHITE): 13 on board, 1 on bar, 1 borne off, in turn (3 moves)")
