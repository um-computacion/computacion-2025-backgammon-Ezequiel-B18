...
class BackgammonUI:
    ...
    def __init__(self):
        ...
        self.bar_rects = {
            1: pygame.Rect(bar_x, BOARD_MARGIN, BAR_WIDTH, SCREEN_HEIGHT / 2 - BOARD_MARGIN),
            2: pygame.Rect(bar_x, SCREEN_HEIGHT / 2, BAR_WIDTH, SCREEN_HEIGHT / 2 - BOARD_MARGIN)
        }
        self.bear_off_rects = {
            1: pygame.Rect(SCREEN_WIDTH - 100, BOARD_MARGIN, 80, SCREEN_HEIGHT / 2 - BOARD_MARGIN),
            2: pygame.Rect(SCREEN_WIDTH - 100, SCREEN_HEIGHT / 2, 80, SCREEN_HEIGHT / 2 - BOARD_MARGIN)
        }
    ...
    def calculate_point_rects(self):
        ...
        for i in range(24):
            if i < 12:  # Bottom half
                ...
                if (11 - i) >= 6:
                    x += BAR_WIDTH
                rect = pygame.Rect(x, SCREEN_HEIGHT - BOARD_MARGIN - POINT_HEIGHT, POINT_WIDTH, POINT_HEIGHT)
            else:  # Top half
    ...
    def draw_board(self):
        ...
        board_rect = pygame.Rect(BOARD_MARGIN, BOARD_MARGIN, SCREEN_WIDTH - 2 * BOARD_MARGIN - 200, SCREEN_HEIGHT - 2 * BOARD_MARGIN)
        pygame.draw.rect(self.screen, LIGHT_BROWN, board_rect)

        bar_x = BOARD_MARGIN + 6 * POINT_WIDTH
        bar_rect = pygame.Rect(bar_x, BOARD_MARGIN, BAR_WIDTH, SCREEN_HEIGHT - 2 * BOARD_MARGIN)
        ...
        for i in range(12):
            ...
            pygame.draw.polygon(self.screen, top_color, [(x, BOARD_MARGIN), (x + POINT_WIDTH, BOARD_MARGIN), (x + POINT_WIDTH / 2, BOARD_MARGIN + POINT_HEIGHT)])
            
            bottom_color = TAN if i % 2 != 0 else DARK_BROWN
            pygame.draw.polygon(self.screen, bottom_color, [(x, SCREEN_HEIGHT - BOARD_MARGIN), (x + POINT_WIDTH, SCREEN_HEIGHT - BOARD_MARGIN), (x + POINT_WIDTH / 2, SCREEN_HEIGHT - BOARD_MARGIN - POINT_HEIGHT)])

        for i in range(12):
            # Top numbers (13-24, left to right)
            num_text_top = str(13 + i)
            text_surface_top = self.font.render(num_text_top, True, WHITE)
            x_top = BOARD_MARGIN + i * POINT_WIDTH + (POINT_WIDTH / 2) - (text_surface_top.get_width() / 2)
            if i >= 6:
                x_top += BAR_WIDTH
            ...
            x_bottom = BOARD_MARGIN + i * POINT_WIDTH + (POINT_WIDTH / 2) - (text_surface_bottom.get_width() / 2)
            if i >= 6:
                x_bottom += BAR_WIDTH
            self.screen.blit(text_surface_bottom, (x_bottom, SCREEN_HEIGHT - BOARD_MARGIN - text_surface_bottom.get_height() - 5))
            ...
            if count > 0:
                ...
                for j in range(count):
                    if i >= 12:
                        ...
                        if (11-i) >= 6:
                            x += BAR_WIDTH
                        y = SCREEN_HEIGHT - BOARD_MARGIN - j * (2 * CHECKER_RADIUS) - CHECKER_RADIUS
                    
                    if j < 5:
                        pygame.draw.circle(self.screen, color, (int(x), int(y)), CHECKER_RADIUS)
                    elif j == 5:
                        num_text = self.font.render(str(count), True, WHITE if player == 2 else BLACK)
                        self.screen.blit(num_text, (x - num_text.get_width() / 2, y - num_text.get_height() / 2 - CHECKER_RADIUS))
                        break
        ...
        for player_id, count in self.game.board.bar.items():
            if count > 0:
                ...
                for i in range(count):
                    if player_id == 1: # White checkers on top part of the bar
                        y = BOARD_MARGIN + 150 + i * (2 * CHECKER_RADIUS) + CHECKER_RADIUS
                    else: # Black checkers on bottom part of the bar
                        y = SCREEN_HEIGHT - BOARD_MARGIN - 150 - i * (2 * CHECKER_RADIUS) - CHECKER_RADIUS
                    pygame.draw.circle(self.screen, color, (int(bar_x), int(y)), CHECKER_RADIUS)
    ...
    def draw_bear_off_area(self):
        ...
        for player_id, count in self.game.board.home.items():
            if count > 0:
                color = WHITE if player_id == 1 else BLACK
                for i in range(count):
                    if player_id == 1: # White checkers (top right)
                        x = self.bear_off_rects[1].centerx
                        y = self.bear_off_rects[1].bottom - (i * CHECKER_RADIUS * 1.5) - CHECKER_RADIUS
                    else: # Black checkers (bottom right)
                        x = self.bear_off_rects[2].centerx
                        y = self.bear_off_rects[2].bottom - (i * CHECKER_RADIUS * 1.5) - CHECKER_RADIUS
                    
                    if y > self.bear_off_rects[player_id].top + CHECKER_RADIUS:
                        pygame.draw.circle(self.screen, color, (int(x), int(y)), CHECKER_RADIUS)
    ...
    def draw_highlights(self):
        ...
        for move in self.highlighted_moves:
            if move == "bear_off":
                highlight_surface = pygame.Surface((self.bear_off_rects[self.game.current_player.player_id].width, self.bear_off_rects[self.game.current_player.player_id].height), pygame.SRCALPHA)
                highlight_surface.fill(HIGHLIGHT_COLOR)
                self.screen.blit(highlight_surface, self.bear_off_rects[self.game.current_player.player_id].topleft)
            else:
                to_point = move
                highlight_surface = pygame.Surface((self.point_rects[to_point].width, self.point_rects[to_point].height), pygame.SRCALPHA)
                highlight_surface.fill(HIGHLIGHT_COLOR)
    ...
    def draw_info_panel(self):
        ...
        panel_rect = pygame.Rect(panel_x, panel_y, 200, SCREEN_HEIGHT - 2 * BOARD_MARGIN)
        pygame.draw.rect(self.screen, LIGHT_BROWN, panel_rect)

        # Draw a separating line
        pygame.draw.line(self.screen, DARK_BROWN, (panel_x - 5, panel_y), (panel_x - 5, panel_y + panel_rect.height), 2)

        if self.game and self.game.current_player:
            player_name = self.game.current_player.name
            player_color = "White" if self.game.current_player.player_id == 1 else "Black"
            ...
            # Bear Off Button
            can_bear_off = self.selected_checker_point is not None and self.game.is_valid_bear_off_move(self.selected_checker_point)
            button_color = LIGHT_BROWN if can_bear_off else DARK_BROWN
            pygame.draw.rect(self.screen, button_color, self.bear_off_button)
            bear_off_text = self.font.render("Bear Off", True, WHITE if can_bear_off else BLACK)
            bear_off_text_rect = bear_off_text.get_rect(center=self.bear_off_button.center)
    ...
    def draw_start_screen(self):
        ...
        self.screen.blit(player1_text, (self.input_boxes["player1"].x + 5, self.input_boxes["player1"].y + 5))
        player1_label = self.font.render("Player 1 (White):", True, BLACK)
        self.screen.blit(player1_label, (self.input_boxes["player1"].x - 200, self.input_boxes["player1"].y + 5))
        ...
        self.screen.blit(player2_text, (self.input_boxes["player2"].x + 5, self.input_boxes["player2"].y + 5))
        player2_label = self.font.render("Player 2 (Black):", True, BLACK)
        self.screen.blit(player2_label, (self.input_boxes["player2"].x - 200, self.input_boxes["player2"].y + 5))
    ...
    def draw_winner_screen(self):
        ...
        self.screen.blit(play_again_text, (self.play_again_button.x + 50, self.play_again_button.y + 15))
    ...
    def handle_click(self, pos):
        ...
        if self.game.board.bar[self.game.current_player.player_id] > 0:
            if self.bar_rects[self.game.current_player.player_id].collidepoint(pos):
                self.selected_checker_point = "bar"
                self.highlighted_moves = self.game.get_valid_moves("bar")
            return
        
        if self.game.board.all_checkers_in_home_board(self.game.current_player.player_id):
            if self.bear_off_rects[self.game.current_player.player_id].collidepoint(pos):
                # Unselect checker to bear off
                self.selected_checker_point = None
                self.highlighted_moves = []
                return
